...
import { useStore } from '../store'; // Zustand로 관리되는 상태를 가져옴
...
const SignUpModal: React.FC = () => {
  ...
  const requestVerificationCode = async () => {
    try {
      const response = await fetch('http://localhost:8080/api/member/mms-number-compare', {
        method: 'POST',
        headers: {
  ...
  return (
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-[#0e2b2f] overflow-y-auto">
      ...
      <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg w-[95%] max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl max-h-full flex flex-col items-center">
        <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-center">회원가입</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col space-y-3 md:space-y-4 w-full">
          ...
          />
          {!isEmailValid && (
            <p className="text-xs text-red-500 mt-1">유효한 이메일 주소를 입력해주세요.</p>
          )}
          {/* 생년월일 입력 필드 */}
          <input
            type="text"
          ...
          <p className="text-xs text-gray-500 mt-1">8자 이상, 영문, 숫자 포함</p>
          {/* 비밀번호 확인 입력 필드 */}
          <div className="relative flex items-center">
            <input
